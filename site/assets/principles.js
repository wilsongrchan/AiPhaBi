...
(function () {
   ...
   * 每個 <dt> 是一步；dt 上的 data-out="B" 表示「這一步淘汰拆法 B」，
   * data-in="C" 表示「這一步判定 C 為正解」。兩個都可以有多個，用逗號分隔。
   * 狀態完全由「目前走到第幾步」推出來（重算而不是累加），所以往回走、
   * 直接跳到某一步、重來，走的都是同一段程式，不會有狀態殘留。 */
  function setupSteppers() {
    document.querySelectorAll('.pr-walk[data-stepper]').forEach(function (dl) {
      if (dl.dataset.ready) return;
      var box = dl.closest('.pr-walkbox');
      var slot = box && box.querySelector('.pr-example[data-stepped]');
      var cards = slot && slot.querySelectorAll('.pr-card[data-label]');
      if (!cards || !cards.length) return;      // 筆畫資料還沒到，下一次 render 再說
      dl.dataset.ready = '1';

      var byLabel = {};
      cards.forEach(function (c) { byLabel[c.getAttribute('data-label')] = c; });

      var dts = [].slice.call(dl.children).filter(function (n) { return n.tagName === 'DT'; });
      var list = function (dt, attr) {
        return (dt.getAttribute(attr) || '').split(',').filter(Boolean);
      };

      var bar = el('div', 'pr-steps');
      var prev = el('button', 'pr-stepbtn');
      var next = el('button', 'pr-stepbtn');
      var now = el('span', 'pr-stepnow');
      prev.type = next.type = 'button';
      prev.textContent = '‹ 上一步';
      ...
      function show(i) {
        at = Math.max(0, Math.min(dts.length - 1, i));
        cards.forEach(function (c) {
          c.className = 'pr-card is-pending';
          c.querySelector('.pr-mark').textContent = '';
        });
        for (var s = 0; s <= at; s++) {
          list(dts[s], 'data-out').forEach(function (L) { set(L, false); });
          list(dts[s], 'data-in').forEach(function (L) { set(L, true); });
        }
        dts.forEach(function (dt, k) {
          dt.classList.toggle('is-now', k === at);
          dt.classList.toggle('is-later', k > at);
        ...
        now.textContent = (at + 1) + '／' + dts.length + '　' + (dts[at].textContent || '');
        prev.disabled = at === 0;
        next.textContent = at === dts.length - 1 ? '↻ 重來' : '下一步 ›';
      }
