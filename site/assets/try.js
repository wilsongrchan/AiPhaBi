...
(function () {
  ...
  /* 跟著打的時候，「一格」最多幾個字。詞組開著、而且文章接下來剛好是收錄詞，
     ...
     （註解裡的例詞一律拿詞庫裡查得到的：〈登鸛雀樓〉開頭的「白日」一度不在詞庫裡，
     照著它讀註解會以為開頁第一格壞掉了。Side B 2026-08-26 已補進去，但規矩不變 ——
     舉例之前先查一次 phrase_dict.json）
  ...
  /* 這串碼還有沒有路：碼表裡還有沒有任何碼「等於它」或「以它開頭」。
     ...
     在打簡碼的最後一鍵被即時頂打斷。有相當一部分簡碼**不是任何一條真碼的前綴**，
     所以少收這兩張表是真的會漏。IME 那邊原本漏了它們，Side B 於 2026-08-26
     （fd8649f）補進 build_index()，兩邊現在是同一套判斷。

     ⚠️ 這裡原本抄了一份「12 條是哪幾條」的名單，2026-09-02 就過期了兩處
     （過 的簡碼 OZ→UZ，因為字根 咼 從 O 搬到 U；學 FXI 新加入那一群），
     總數也從 63 變 67。名單不要抄 —— 要看現在是哪幾條就現算：

       node -e "const d=require('./site/assets/dict.json'),r=Object.keys(d.codes).sort();
       console.log(Object.entries(d.short).filter(([c])=>
         !r.some(k=>k.startsWith(c))).map(([c,ch])=>ch+' '+c.toUpperCase()).join('、'))"
