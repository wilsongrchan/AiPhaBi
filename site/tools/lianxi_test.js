 ...
 *
 * 測的是 lianxi.js 的 buildPool／inScope／scoped／pickQuestion —— 也就是「這一輪
 * 該問哪一條字根、拿哪個字當例字」。畫面的部分（田字格、字母疊上去）用無頭瀏覽器
 * 看得出來，出題邏輯看不出來：它是機率性的，跑一次正確不代表下一次正確。
 *
...
/* ── 替身：一張小小的字根表 ─────────────────────────────────────────── */
// 兩個字母、三條字根。O 那條最常見（count 90），A 的兩條分別是優等與三等。
let DATA = {
  letters: [
    { letter: 'O', groups: [
      { desc: '「口」字及類似字形', tier: 'primary', shapes: [
        { src: '口', st: [0, 1, 2], span: 'whole', count: 90,
          ex: [{ c: '口', st: [0, 1, 2] }, { c: '哪', st: [0, 1, 2] }, { c: '合', st: [3, 4, 5] }] },
      ] },
    ] },
    { letter: 'A', groups: [
      { desc: '「夕」字及類似字形', tier: 'primary', shapes: [
        { src: '夕', st: [0, 1, 2], span: 'whole', count: 50,
          ex: [{ c: '外', st: [0, 1, 2] }] },
      ] },
      { desc: '很罕見的那一條', tier: 'tertiary', shapes: [
...
let opts = { scope: 'primary', order: 'common' };
eval(cut('buildPool'));
eval(cut('inScope'));
eval(cut('scoped'));
...
function reset() { mastered = {}; lastKey = ''; opts = { scope: 'primary', order: 'common' }; }
...
console.log('範圍');
reset();
ok('優等：三等那條不出題', scoped().length === 2, scoped().length);
opts.scope = 'all';
ok('全部：三條都出題', scoped().length === 3, scoped().length);
