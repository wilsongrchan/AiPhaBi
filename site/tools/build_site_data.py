...
def build_practice_hints(paras, codes, zigen_raw, max_rule):
    """參考文章每個字的「這一段是哪一個字根」—— 給 `/` 提示鏈用。
    ...
    want = {c for para in paras for c in para if "\u4e00" <= c <= "\u9fff"}
    out, unmatched = {}, 0
...
def build_practice(main, codes, zigen_raw, max_rule):
    """試打頁的參考文章：site/content/practice.md，`---` 底下的純文字。

    給〈線上試打〉右邊的田字格用 —— 田字格裡是下一個要打的字，跟著這篇文章走。
    檔案不存在或空的就回傳 None，那一整塊（參考文章＋田字格）不出現，
    試打框本身照常能用。
    ...
    hans = [c for p in paras for c in p if "\u4e00" <= c <= "\u9fff"]
    missing = sorted({c for c in hans if c not in main})
    # 字形跟文章放在同一個檔：田字格一次只畫一個字，但那個字隨時會換，
    # 沒辦法逐字抓。抓不到 graphics.txt 就沒有 glyphs，田字格退回系統字型 ——
    # 練習照樣能練，只是格子裡不是筆畫輪廓。
    glyphs = _practice_glyphs(set(hans))
    hints = build_practice_hints(paras, codes, zigen_raw, max_rule)
    return {
        "note": "generated by site/tools/build_site_data.py — 內容見 site/content/practice.md",
        "title": "背影",
        "author": "朱自清",
        "source": "zh.wikisource.org",
        "license": "公有領域（作者 1948 年逝世，1931 年前出版）",
        "paras": paras,
        "chars": len(hans),
        "missing": missing,
        # 字形資料的授權跟 glyphs.json 完全一樣，而且這個檔也會隨網站發佈出去，
        # 所以義務也一樣：ARPHICPL.txt 必須跟著發佈，這兩欄不要拿掉。
        ...
        "glyph_modifications": (
            "自 graphics.txt 取出本篇參考文章用得到的字元，保留其 strokes 欄"
...
def main():
    ...
    practice = build_practice(dict_out["main"], codes, zigen_raw, max_rule)
    ...
    if practice:
        miss = practice["missing"]
        print(f"practice.json 參考文章〈{practice['title']}〉"
              f"{len(practice['paras'])} 段 / {practice['chars']} 字"
