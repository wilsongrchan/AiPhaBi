...
def build_lianxi_full(zg, warn, limit=None, per_level=8):
        ...
        for grp in L.get("groups", []):
            for sh in grp.get("shapes", []):
                ...
                if not segs or not segs[0]:
                    continue
                made.append({"c": e0["c"], "L": letter, "g": [segs[0]]})
    ...
    return {"note": "generated by site/tools/build_site_data.py — 完整版：涵蓋整張字根表，"
            ...
            "levels": 1, "pass": min(per_level, len(made)), "questions": made,
            "glyphs": glyphs}
...
def main():
    ...
    # 完整版預覽：先出 10 題看看長什麼樣子（Wilson 2026-09-24），確定方向了
    # 再把 limit 拿掉、出滿整張字根表。見 build_lianxi_full 的說明。
    lianxi_full = build_lianxi_full(zg, warn, limit=10)
    ...
    if lianxi_full:
        ...
        cs = [q["c"] + q["L"] for q in lianxi_full["questions"]]
        print(f"lianxi_full.json 〈字根練習〉完整版預覽 {len(cs)} 題 / {kb:.0f} KB"
              f"（等距抽樣自整張字根表，見 build_lianxi_full 的 limit；{' '.join(cs)}）")
