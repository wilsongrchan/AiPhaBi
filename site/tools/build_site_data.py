...
"""Generate the data files the public site needs, from the repo's own source of truth.
    ...
    site/assets/gongnengjian.json 部件／獨立字共用一條碼的清單，給 gongnengjian.html 用
...
PRINCIPLE_WRONG = {
    ...
    # 目字旁原則（凌駕孤筆略過）的最小對照組：睛＝目（最左）＋青，腈＝月（最左，
    # 肉月旁）＋青，右半邊完全同形（青＝F＋D），左半邊只差目比月多一橫。睛的錯誤
    # 拆法就是把那一橫（index 4）當孤立橫劃略過，變成跟腈一模一樣的 DFD——這正是
    # codes.json 裡 睛 的 compliantWhy 講的「跟月字旁 D 做區別」要避免的碰撞。
    "睛": [{"code": "DFD", "groups": [[0, 1, 2, 3], [5, 6, 7, 8], [9, 10, 11, 12]],
            "skip": [4]}],
    # 腈沒有錯誤拆法可畫——月字旁本身就沒有目那多出來的一橫，DFD 就是它唯一、
    # 正常的拆法，只是拿來跟睛對照。
    #
    # 相／想這一對不是目字旁例外，是孤筆略過原則本身「首筆或末筆不略過」那句的
    # 對照：相＝木＋目，目的那一橫剛好是全字最後一筆，本來就不略過（TDI）；
    # 想＝相＋心，同一個目多了心接在後面，那一橫既不是最左（木在它左邊）也不是
    # 全字最後一筆，就正常略過（TDW，index 8 的缺口）。兩個都沒有錯誤拆法可畫。
}
...
def build_practice(main, codes, zigen_raw, max_rule, conv_chars, t2s):
    ...
    `t2s` 是繁 → 簡單字對照：試打頁簡體開著時，田字格教的是文章原字（繁體，
    手寫都是繁體）的簡體對應字，不是原字本身（見 try.js 的 targetChar／
    site.js 的 simplifyChar，2026-09-08）。所以 glyphs／segs 除了文章原文的字，
    還要多收每個原字對到的簡體字——查得到形碼的才收得到 segs，收不到的話
    田字格會退回系統字型純文字，跟現有「沒取碼」的字一樣，不是新狀況。
    """
    ...
    # 簡體開著時田字格要教的是簡體字自己的形狀（見上面 docstring），所以
    # glyphs／segs 收字要多收一輪：每個原字（繁體）對到的簡體字，不同字才收，
    # 同形字（一、人…）本來就在 all_hans 裡，收兩次沒有意義。
    simp_extra = {t2s[c] for c in all_hans if t2s.get(c) and t2s[c] != c}
    want = all_hans | simp_extra
    # 字形跟文章放在同一個檔：田字格一次只畫一個字，但那個字隨時會換，
    # 沒辦法逐字抓。抓不到 graphics.txt 就沒有 glyphs，田字格退回系統字型 ——
    # 練習照樣能練，只是格子裡不是筆畫輪廓。
    glyphs = _practice_glyphs(want)
    hints = build_practice_hints(want, codes, zigen_raw, max_rule)
    ...
    return {
        ...
        **_tw_credit(glyphs),
        ...
        # ⚠️ 查 want 不是 all_hans：簡體字是不是約定字要看它自己（个 是不是約定字
        # 跟 個 是不是約定字，是兩件各自獨立的事，不能借用原字的答案）。
        "conv": sorted(want & conv_chars),
    }
...
def build_lianxi(picks, codes, max_rule, warn, per_level=8):
    ...
    ⚠️ 揭曉時**不再列出取形意圖／取自哪個字／整串碼**（Wilson 2026-09-01：
    每答對一次頁面就長出一塊，畫面跟著彈；那些東西在〈字根表〉看就好）。
    所以這裡也不再呼叫比對器 —— 沒有人用的資料就不要產。

    ⚠️ 哪幾筆屬於哪一條字根一律**現算**（codes.json 的 segments），這一頁不手寫
    任何一條「某某字根是某字母」—— 那是這個專案最會過期的東西。

    ⚠️ 同一個字母在一個字裡出現好幾次（檢 的兩個 O、巡 的三個 L）算**同一題**，
...
def _split_letters_even(letter_counts, n_buckets):
    """把 [(字母, 這個字母有幾題), …]（照 A→Z 出現順序）切成 n_buckets 段連續
    範圍，段界盡量讓每段的題數加總接近平均——但只能切在字母跟字母之間，
    不能把同一個字母拆成兩段。

    用來把 377 個字根按字母分成「A–F」「G–J」這種幾段，讓完整模式一次只
    面對一小段（Wilson 2026-09-24：「so it doesn't feel like they need to
    learn all 300 at once」），段數固定但段界跟著實際題數分布走，不是機械式
    每 5 個字母切一刀——K 底下有 34 題，比 B 的 3 題多十倍，機械式切法會讓
    某幾段特別重。
    """
    n = len(letter_counts)
    ...
    for k in range(1, n_buckets):
        want = total * k / n_buckets
        best_j, best_diff = None, None
        # 至少要留 (n_buckets - k) 個字母給後面剩下的段，不然後面會有段是空的
        for j in range(last + 1, n - (n_buckets - k)):
            diff = abs(cum[j] - want)
        ...
        last = best_j
    bounds = [-1] + cuts + [n - 1]
    return [letter_counts[bounds[i] + 1: bounds[i + 1] + 1] for i in range(len(bounds) - 1)]


def build_lianxi_full(zg, warn, limit=None, per_level=8, n_buckets=5):
    """〈字根練習〉的「完整版」題庫：不是 Wilson 手挑的一小批，是**整張字根表**——
    377 個字根各出一題（Wilson 2026-09-24：字根表逛完之後，回頭真的把整張表
    考過一遍，不是只練歡迎模式那幾條）。

    例字跟哪幾筆屬於這個字根，直接借用 build_zigen() 已經算好的 ex（見
    _examples()），不重新讀 codes.json、不重新判斷筆序——那一套規則（own／
    claims／owners，見 _examples 的說明）已經解決了「同一個字裡好幾段都符合
    但屬於不同字根」這種歧義，這裡另外寫一套只會重蹈覆轍。

    ⚠️ 這裡沒有「提示／要轉才像」那些判斷——那是 Wilson 手挑歡迎模式時才做的
    決定（見 build_lianxi 的 hint／_lianxi_tf：「這個字根要轉幾度才看得出來」
    是他讀那個形狀讀出來的，不是資料裡有的東西）。377 個字根不可能逐一手判，
    所以完整版的題目一律照歡迎模式第一關的問法：「這個字根像哪個字母？」，
    不會出現旋轉、翻轉那類題目（轉／翻的按鈕仍然留著，只是不強制、也不自動轉，
    見 lianxi.js 的 ALWAYS_SPIN）。

    ⚠️ 完整模式不用「第一關」「第二關」這種編號，也不顯示「幾／幾題」的進度條
    （Wilson 2026-09-24：一號到底像在交代必須依序打完的關卡，跟這裡「隨你從
    哪一段字母開始、想學多少學多少」的用法不搭）。改成 level_names（"A–F" 這種
    字母範圍字串，跟 questions 的 lv 對應），lianxi.js 看到這個欄位就換一種
    畫法——見那邊 paintLevel／LEVEL_NAMES 的說明。

    limit：先出幾題就好（Wilson 2026-09-24：「lets just do 10 questions now so
    i can take a look at it」）。給 None 就是全部字根都出。用等距抽樣、不是
    隨機也不是取前 N 個——這樣 10 題會平均分散在 A 到 Z，不會全部擠在字母表
    最前面那幾個字母。

    每題帶一個 "d"（這個字根的取形意圖，即 grp["desc"]，build_zigen() 已經算
    好的那句話）——完整模式的提示／揭曉都會用到，見 lianxi.js 的 hint()／
    reveal()。歡迎模式的 build_lianxi() 沒有這個欄位，是刻意的：Wilson
    2026-09-01 決定歡迎模式揭曉不列取形意圖，兩份資料不同形狀，前端才分得出
    現在是哪一種。desc 還沒寫的（build 印「⚠️ 還沒寫取形意圖」那種）就不帶
    這個鍵，前端會整段跳過。
    """
    made = []
    for L in zg.get("letters", []):
        letter = L.get("letter")
        for grp in L.get("groups", []):
            desc = (grp.get("desc") or "").strip()
            for sh in grp.get("shapes", []):
                ex = sh.get("ex") or []
                if not ex:
                    continue
                e0 = ex[0]
                ...
                if not segs or not segs[0]:
                    continue
                q = {"c": e0["c"], "L": letter, "g": [segs[0]], "_letter": letter}
                if desc:
                    q["d"] = desc
                made.append(q)

    if not made:
        warn.append("〈字根練習〉完整版：一題都生不出來")
        return None

    if limit and limit < len(made):
        step = len(made) / limit
        made = [made[int(i * step)] for i in range(limit)]

    # 依字母分段——見 _split_letters_even 的說明。段界只看**這一批實際出現的
    # 題目**（不是全部 377 個字根的分布），limit 還沒拿掉的預覽階段一樣算得出
    # 合理的段。
    letter_order, letter_count = [], {}
    for q in made:
        l = q["_letter"]
        if l not in letter_count:
            letter_order.append(l)
        letter_count[l] = letter_count.get(l, 0) + 1
    buckets = _split_letters_even([(l, letter_count[l]) for l in letter_order], n_buckets)

    bucket_of, level_names = {}, []
    for i, seg in enumerate(buckets):
        level_names.append(seg[0][0] if len(seg) == 1 else f"{seg[0][0]}–{seg[-1][0]}")
        for l, _ in seg:
            bucket_of[l] = i
    for q in made:
        q["lv"] = bucket_of[q.pop("_letter")]

    glyphs = _practice_glyphs({q["c"] for q in made})
    missing = sorted({q["c"] for q in made if q["c"] not in glyphs})
    if missing:
        warn.append(f"〈字根練習〉完整版：{len(missing)} 個字沒有字形資料"
                    f"（{''.join(missing)}），那幾題畫不出來")
    made = [q for q in made if q["c"] in glyphs]
    if not made:
        warn.append("〈字根練習〉完整版：字形資料缺太多，一題都畫不出來")
        return None

    return {"note": "generated by site/tools/build_site_data.py — 完整版：涵蓋整張字根表，"
                     "不是手挑清單（手挑清單是 lianxi.json／site/content/lianxi.md）",
            "levels": len(level_names), "level_names": level_names,
            "pass": min(per_level, len(made)), "questions": made, "glyphs": glyphs}
...
PHRASE_TWO = [
    ...
    #
    # 排法是**由簡到繁**：先看兩個字都沒有捷徑、就只是把主碼接起來的樣子（不去、
    # 白色），再看捷徑怎麼讓同一個詞多出好幾條路（香港、我的、教授）。原本三個
    # 例子全都有捷徑，等於從沒示範過最基本的那一種（Wilson 2026-08-26 指出要有
    # 2+2、2+3 這類長度組合）。
    ("不去", "兩個字都只有主碼、各 2 碼，接起來就是 4 碼"),
    ("白色", "2 碼接 3 碼 —— 兩個字碼長不一樣照接，中間不必分隔"),
    ("香港", "港 沒有約定簡碼，落到三簡碼 ZHZ —— 首尾相接就是最短的那條"),
...
# 五字以上那個例子還多一條要求：**五個相關的字都不能有兼容碼**（Wilson
# 2026-08-26）。有兼容碼的字會替每一式各換出好幾條簽名，卡片上一次冒出四五條碼，
# 「兩式」那件事就被淹掉了 —— 聯合國教科文組織 正是這樣（教 的兼容碼讓第一式
# 長出 NAOT／NAOF，跟第二式的 NAOG 擺在一起根本分不出誰是誰）。醉翁之意不在酒
# 每一式只有一條：IVQI／IVQW，差在第四碼，一眼看得出前三碼是共用的。
# 挑一句成語而不挑國家全名或國際組織（Wilson）：那兩類會被讀成立場，見
# CORPUS_GROUPS 的註解；而這一句兩岸三地都認得，又本來就在成語那一節裡。
# （同樣乾淨的備選：斯德哥爾摩 HJIV／HJIR、萬國郵政聯盟 HOJF／HOJB。）
# ⚠️ 這幾條規矩底下有 _si4_example_warn() 在建置時實際核，別只靠讀這段註解：
#    巴拿馬運河 就是這樣被擋下來的（馬 是約定字，Wilson 一眼看出來，我沒有）。
# 另一個例外是 長白山：它示範的不是「取首碼」，而是**末字只有一碼時會怎樣**——
# 山 的碼就是 W，首碼末碼是同一個字母，四碼結尾因此變成 WW。看過 劉德華 那種
# 「末字取兩碼」之後，這是同一條規則撞到一碼字的樣子，不寫出來會以為是錯的
# （Wilson 2026-08-26）。所以它必須排在 劉德華 後面，先規則、後特例。
PHRASE_SI4 = [
    ("劉德華", "三字：前兩字首碼 ＋ 末字的首碼和末碼"),
    ("長白山", "末字只有一碼（山＝W）：首碼末碼同一個字母，結尾就成了 WW"),
    ("守株待兔", "四字：各取首碼"),
    ("醉翁之意不在酒", "五字或以上：取前四字首碼；或取前三字首碼及末字首碼"),
]
...
def _note_code_warn(pc):
    """例詞的說明文字裡寫死的碼，還對不對得上 codes.json。

    CLAUDE.md 的規矩：網站不要手抄碼，抄了就會過期、還會被人念出來。可是有些
    說明就是得指名道姓（「主碼 QM，兼容碼 IM」比「它有一條兼容碼」清楚得多），
    所以改成寫得下、但建置時核一次：說明裡每個看起來像碼的大寫字串，都必須真的
    是這個詞裡某個字打得出來的一條碼。對不上就叫，別讓它默默過期。
    """
    out = []
    for w, note in list(PHRASE_TWO) + list(PHRASE_SI4):
        ok = set()
        for ch in w:
            ...
            if mc:
                ok.add(mc.upper())
                if len(mc) >= 4:
                    ok.add((mc[0] + mc[1] + mc[-1]).upper())      # 三簡碼
            for a in pc.alt_codes.get(ch, ()):
                ok.add(a.upper())
            sh = pc.short_rev.get(ch)
            if sh:
                ok.add(sh.upper())
        # 四碼快打的簽名跟它的尾巴也算數：長白山 的說明講的是「結尾成了 WW」，
        # 那是四碼的後兩碼，不是任何一個字的碼。
        for _pos, sigs in pc.si4_forms(w):
            for sig in sigs:
                u = sig.upper()
...
def _si4_example_warn(rules, pc):
    """四碼快打的例詞違不違反自己那幾條挑選規矩 —— 建置時核，不要靠讀註解。

    三次都是同一種錯：規矩寫在註解裡，例詞是另一個時間挑的，兩邊對不上也沒人
    發現（人山人海 的一碼字、聯合國教科文組織 的兼容碼、巴拿馬運河 的 馬 是約定字）。
    只警告不擋 —— 挑例子是編輯判斷，程式沒有資格否決，但它有資格提醒。
    """
    conv_rule = next((r for r in rules.get("rules", []) if r["id"] == "convention"), None)
    conv = {c["c"] for g in (conv_rule.get("groups", []) if conv_rule else [])
            for c in g.get("chars", []) if c.get("c")}
    out = []
    for w, _note in PHRASE_SI4:
        rel = list(w[:4]) + [w[-1]]                 # 四碼真正取到的那幾個字
        for c in dict.fromkeys(rel):
            if (w, c) in SI4_EXAMPLE_OK:            # 刻意的例外，理由見那張表
                continue
            if c in conv:
                out.append(f"四碼例詞「{w}」：{c} 是約定字，碼不是照筆順拆出來的，"
                           f"示範不了「取首碼＝取第一條字根」")
            elif len(pc.char2code.get(c, "")) < 2:
                out.append(f"四碼例詞「{w}」：{c} 只有一碼，看不出取的是首碼還是整條碼")
            if len(w) >= 5 and pc.alt_codes.get(c):
                out.append(f"四碼例詞「{w}」：{c} 有兼容碼，兩式會各長出好幾條簽名，"
                           f"「兩式」那件事會被淹掉")
    return out
...
def build_phrases(codes, rules, max_rule, zigen_raw):
    ...
    ship = _ship_words()
    ship_si4 = _ship_si4()
    warn += _si4_example_warn(rules, pc)
    warn += _note_code_warn(pc)
    ...
    for w, note in PHRASE_SI4:
        ...
        if ship_si4 is not None:
            ...
            entry["ok"] = True
        # 「同碼的還有 …」那一句 Wilson 2026-08-26 拿掉了：卡片要講的是碼怎麼
        # 推出來，撞碼是另一件事，混在同一張卡裡會把規則本身蓋掉。ship_si4 還是
        # 要查（上面拿它核對簽名對不對得上碼表），只是不再輸出這個欄位。
        si4.append(entry)
...
CORPUS_GROUPS = [
    ...
    # ⚠️ 節名必須跟 data/phrases_*.txt 裡的小節標題**逐字相同**（_sec_key 取
    #    第一個空白或全形括號之前那一段）。Side B 改過節名就會整節掉出網站，
    #    建置時會印一行 ⚠️ 說「找不到這一節」——看到就是要來改這裡。
    #    「香港三大區域」2026-08-26 被改成「香港區域 Hong Kong regions」。
    ...
    ("機構與組織", [("orgs", ("國際組織", "-中國機關／政黨", "-台灣機關", "-香港機關")),
                    ("common", ("-政府機構", "大學"))]),
    ("品牌與公司", [("brands", None)]),
    # 兩節「練習用詞」整節算數字、不自動舉例（前綴 -）：那些詞是 Side B 2026-08-26
    # 為了讓試打頁練得到詞組／四碼快打而加的功能性詞組（〈水調歌頭〉七個詞、
    # 〈登鸛雀樓〉的「白日」），不是挑出來示範成語的 —— 白日 尤其不是成語。
    # 照樣算進「收錄 N 個詞」，只是不進例子列表。
...
# 本身更有代表性。同樣的理由，也刻意混進動詞、副詞（決定／運動／突然），不要讓
# 整組看起來只有名詞、只有職場詞——詞性跟主題都要雜（Wilson 2026-08-30：「不想
# 讓人以為只收名詞或只收職場用語」）。原本這裡還放了「謝謝」，但跟「你好」同樣
# 是招呼語，兩個放在一起讀起來像同一組，換成「決定」（Wilson：兩個是 repeat）。
...
#   我們 47.9萬　電影 10.3萬｜也可以 8.0萬　客戶 6.0萬　運動 58476　決定 53056
#   一個人 4.7萬　突然 43913　流量 3.2萬　你好 2.1萬｜便宜 15487　水果 12988
#   高興 9469　盒子 6257　玫瑰 4732　最後一次 4529　龐大 4257　熊貓 2622｜
#   信封 1285　加班費 1227　複診 864
DAILY_PICKS_N = 21
DAILY_PICKS = ("我們", "電影", "也可以", "客戶", "運動", "決定", "一個人",
               "突然", "流量", "你好", "便宜", "水果", "高興", "盒子", "玫瑰",
               "最後一次", "龐大", "熊貓", "信封", "加班費", "複診")
...
def main():
    ...
    if tfp.exists():
        ...
        if tf_total:
            ...
            # ⚠️ 這裡**不要**先四捨五入到 2 位小數再存——2026-09-24 覆蓋率衝到
            # 99.9986% 時，round(x, 2) 本身就先進位成 100.00，把「還沒到 100%」
            # 這個事實在存進 JSON 之前就弄丟了，site.js 那邊「一律無條件捨去、
            # 絕不講成 100%」的防呆（Wilson 2026-09-02 的指示，見那邊註解）
            # 因此完全失靈，實際顯示變成「約 100.0%」。存原始精度，捨去交給
            # site.js 在**顯示的那一刻**做。
            stats["coverage"] = {
                "all": sum(n for c, n in tf.items() if c in codes) / tf_total * 100,
                "everyday": (clean_cov / clean_total * 100
                             if clean_total else None),
                "corpus": tf_total,
            }

    # 重碼率：「只打常用字」＋「不打簡體」兩個開關都打開後，還打得出來的那批字裡，
    # 有多少字跟別的字共用同一個主碼。這是對外會被引用的數字，所以定義寫死在這裡、
    # 每次重算，跟 stats.html〈碼表分析〉頁「打開『只打常用字』和『不打簡體』後」
    # 那個切法（commonNoSimp）用同一套定義，三處算出來的數字才對得上：
    #   常用字白名單＝ rime/lua/aiphabi_data.lua 的 M.common（跟 /api/common-whitelist
    #     同一份）；簡體專屬字＝ data/opencc.json 的 s2t 表收錄、但不在
    #     data/dual_use_merged.json 歸併字名單裡的那些（后／干／咸這種本身也是傳承字
    #     的不算簡體專屬，見 server.py 的 _load_simp_only）。
    #   部件字（componentOnly，扌／爿／丬…）留在分母裡，但不參與重碼判斷、也不算進
    #   重碼字數——它們打主碼看不到，要加 ` 才打得到，跟別的字比對重碼沒有意義。
    lua_text = (RIME / "lua" / "aiphabi_data.lua").read_text("utf-8") \
        if (RIME / "lua" / "aiphabi_data.lua").is_file() else ""
    m = re.search(r"\nM\.common = \{(.*?)\n\}", lua_text, re.S)
    common_whitelist = set(re.findall(r'\["(.+?)"\]', m.group(1))) if m else set()
    dual_use = set()
    dual_use_path = DATA / "dual_use_merged.json"
    if dual_use_path.exists():
        dual_use = set(json.loads(dual_use_path.read_text("utf-8")).get("chars", []))
    simp_only = {c for c in s2t_raw if c not in dual_use}

    pool = [c for c in codes if c in common_whitelist and c not in simp_only]
    groups = {}
    for ch in pool:
        if codes[ch].get("componentOnly"):
            continue
        groups.setdefault(shorten(codes[ch]["code"], max_rule), []).append(ch)
    clashed = sum(len(v) for v in groups.values() if len(v) > 1)
    if pool:
        stats["clash"] = {"pool": len(pool), "chars": clashed,
                          "pct": round(clashed * 100 / len(pool), 1)}
    ...
    # 完整版：Wilson 2026-09-24 看過 10 題的預覽後定案，出滿整張字根表
    # （limit=None）。見 build_lianxi_full 的說明。
    lianxi_full = build_lianxi_full(zg, warn, limit=None)
    ...
    gongnengjian = build_gongnengjian(codes)
    ...
    practice = build_practice(dict_out["main"], codes, zigen_raw, max_rule, conv_chars, t2s)
    ...
    if lianxi_full:
        (OUT / "lianxi_full.json").write_text(
            json.dumps(lianxi_full, ensure_ascii=False, separators=(",", ":")), "utf-8")
    ...
    (OUT / "gongnengjian.json").write_text(
        json.dumps(gongnengjian, ensure_ascii=False, separators=(",", ":")), "utf-8")
    ...
    if lianxi:
        ...
        if alt_qs:
            print("  （其中 " + "、".join(f"{q['c']} {q['L']} 取自兼容碼 {q['alt']}"
                                          for q in alt_qs) + "）")
    if lianxi_full:
        kb = (OUT / "lianxi_full.json").stat().st_size / 1024
        by_lv = {}
        for q in lianxi_full["questions"]:
            by_lv.setdefault(q["lv"], []).append(q["c"] + q["L"])
        print(f"lianxi_full.json 〈字根練習〉完整版 {len(lianxi_full['questions'])} 題 / "
              f"{kb:.0f} KB　涵蓋整張字根表，見 build_lianxi_full")
        for i, name in enumerate(lianxi_full["level_names"]):
            print(f"  {name}：{len(by_lv.get(i, []))} 題")
    ...
    n_comp = sum(len(g["components"]) for g in gongnengjian["codes"])
    print(f"gongnengjian.json {len(gongnengjian['codes'])} 條碼有部件旗標"
          + f"（共 {n_comp} 個部件）")
