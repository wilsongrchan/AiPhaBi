...
"""Generate the data files the public site needs, from the repo's own source of truth.
    ...
    site/assets/gongnengjian.json 部件／獨立字共用一碼的字母清單，給 gongnengjian.html 用
...
def build_gongnengjian(codes):
    """〈功能鍵〉頁第 4 節（部件）的資料：哪些字母的純一碼被「獨立成字」跟
    「純部件、不能單獨成字」兩批字共用。Side A 用 codes.json 的 componentOnly
    旗標標記後面那批（見 2026-09-08 commit 5aee6b9），這裡只是照旗標分組——
    不手抄清單，因為 Wilson 說了這批字以後還會再增加（見 CLAUDE.md 附的
    memory）。main 是「同一個字母、沒被標 componentOnly」的字：目前每個字母
    最多一個，但不保證——J、U 兩個字母現在完全沒有 main（純部件，見下面印出
    的警告），components 是被標 componentOnly 的那些。
    """
    main_by_letter, comp_by_letter = {}, {}
    for ch, rec in codes.items():
        ...
        if not f or len(f) != 1:
            continue
        (comp_by_letter if rec.get("componentOnly") else main_by_letter).setdefault(f, []).append(ch)

    letters = []
    warn = []
    for L in sorted(comp_by_letter):
        main = main_by_letter.get(L, [])
        if not main:
            warn.append(f"功能鍵頁：字母「{L}」目前沒有獨立成字的一碼字，"
                        f"打「{L}」本身查不到字，只有 `{L}` 查得到部件"
                        f"（{'、'.join(comp_by_letter[L])}）")
        letters.append({"letter": L, "main": main, "components": comp_by_letter[L]})
...
def main():
    ...
    n_comp = sum(len(g["components"]) for g in gongnengjian["letters"])
    print(f"gongnengjian.json {len(gongnengjian['letters'])} 個字母有部件旗標"
          + f"（共 {n_comp} 個部件）")
