...
- **Switched to `reset: 1` (default on) 2026-08-17** — same fix, same reason as `aiphabi_phrase`
  above: on Hamster, `switcher/save_options` persistence is unreliable (toggle it on, app-switch or
  redeploy, it's back to the schema default) even though `aiphabi_left_short` is correctly listed
  in `default.custom.yaml`'s `switcher/save_options` and had no `reset:` forcing it — this is a
  Hamster-side limitation, not a config bug on our end (confirmed: user toggled it on, it reverted
  after the next zip re-import). Trade-off is the same one already accepted for 詞組連打: forcing
  a fixed default (on) beats relying on persistence that mobile can't be trusted to keep, at the
  cost of also forcing it on for desktop/Squirrel users on every redeploy (persistence *is*
  reliable there, so this is a regression for anyone on macOS who explicitly wants it off — no
  per-platform way to have both with a single schema file). Mirrored in `aiphabi_plus.schema.yaml`
  too (hand-maintained, not generated by `build_rime.py`) for consistency between the two schemas.
