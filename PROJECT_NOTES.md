...
> **This has already happened once.** Merge `04441f1` ("Merge remote-tracking branch 'origin/main'")
> has parents `902f5b7` (391 zigen, R 厂#0,1 count 147) and `37ea2d9` (400 zigen, count 152) — and
> its result has **count 97**, a value present in *neither* parent, with the R 捺劃 exemplar
> `大#2` (count 323, 23 chars in `seen`) gone. The later "刪除 14 條孤兒字根" cleanup (`cff3be4`)
> then pruned zigen that may have looked orphaned only because of that merge.
