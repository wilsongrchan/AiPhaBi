...
def seed_default_options(path):
    ...
    off_by_default = ["aiphabi_t2s", "aiphabi_s2t", "aiphabi_no_simp", "aiphabi_common_only",
                      "aiphabi_autocommit", "aiphabi_short3",
                      "full_shape", "ascii_punct"]
...
def main():
    ...
    # 「誰需要」。
    # 部件字的碼位是 `k 不是 k（見上面主迴圈）——提示（markHints／萬用鍵）要秀實際打得到的碼。
    char2code = {c: ("`" if rec.get("componentOnly") else "") + shorten(rec["code"], max_rule).lower()
    ...
    # 幫忙種一次初始值，見上面 seed_default_options。
    short_switch = ""
    if shortcode:
    ...
    if short3:
        short3_switch = ("  - name: aiphabi_short3            # 三簡碼：頭兩碼+末一碼，當 AB`C 查\n"
                          "    states: [ 三簡碼關, 三簡碼開 ]\n")
    # 左簡碼開關：跟 aiphabi_phrase 同一個理由改成 reset:1 預設開（2026-08-16）——
    # 手機（Hamster）上「記住選擇」不可靠，使用者切 app／重新部署常被打回預設值，
    # 不是這裡的邏輯問題。與其讓使用者每次都要重新開一次，不如把預設值直接定在「開」，
    # 這樣被重置回預設時至少停在使用者想要的那個狀態（同樣的取捨也套用到桌面版）。
    left_switch = ""
    if leftshort:
...
都靠鼠鬚管內建的 librime-lua，裝好就能用；點選單列鼠鬚管圖示就能個別勾選開關（`sync.sh` 安裝時預設開的只有：同類字／偏旁碼／輸入容錯／約定簡碼／詞組連打；其餘全部預設關，包括三簡碼／只打常用字／自動上屏）：
...
`default.custom.yaml`、`hamster.custom.yaml`。打包成 zip 時**不要包住
